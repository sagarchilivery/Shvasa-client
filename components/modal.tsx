...
import React, { Children, FormEvent, Fragment, useState } from "react";
  ...
  const HandleSubmit = async (e: FormEvent) => {
    try {
      const createRes = await axios.post(
        "http://localhost:1337/support-tickets"
      );
      ...
      <Dialog.Panel className="max-w-[500px] bg-[#282828] border max-h-[75vh] overflow-hidden h-full mx-auto w-full">
        <form onSubmit={HandleSubmit} className="">
          ...
          <div className="flex items-center justify-center w-full">
            <button className=" border px-4 py-1 rounded-md">Submit</button>
          </div>
        </form>
      </Dialog.Panel>
    </Dialog>
